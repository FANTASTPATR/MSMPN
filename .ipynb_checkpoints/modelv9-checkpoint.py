'''
此版本增加了pearson附属矩阵，且尝试用4个skip-connect，snapshots
'''
import torch
...
class Model(nn.Module):
    ...
    def forward(self, inputs):
        if inputs.shape[1] != self._chp[-1]:
            pad_len = self._chp[-1] - inputs.shape[1]
    ...
    #             offset += adj_mx.shape[0]
    #         else:
    #             hstack_unit = torch.cat((adj_mx, torch.eye(adj_mx.shape[0]), torch.eye(adj_mx.shape[0])), dim=-1)
    #             rst[offset:offset + hstack_unit.shape[0], offset:offset + hstack_unit.shape[1]] = hstack_unit
    #             offset += adj_mx.shape[0]
    #     return rst
    ...
    @staticmethod
    def _generate_sparse_pearson_mx(file_path, ratio=0.98):
