'''
此版本增加了pearson附属矩阵，且尝试用3和snapshots
'''
import torch
    ...
    def _combine_adj_forward(adj_mx, snapsht_len):
        ...
        for v_idx in range(snapsht_len):
            if v_idx == snapsht_len - 1:
                ...
                connect_unit = torch.cat([torch.eye(adj_mx.shape[0]) for _ in range(snapsht_len - 1 - v_idx)], dim=-1)
                hstack_unit = torch.cat((adj_mx, connect_unit), dim=-1)
